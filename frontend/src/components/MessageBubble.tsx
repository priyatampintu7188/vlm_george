...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message }) => {
    ...
    return (
        <div className={`w-full py-6 transition-colors duration-300 ${isUser ? '' : 'bg-[var(--bubble-ai)]'}`}>
            <div className={`max-w-3xl mx-auto px-4 flex gap-4 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                
                <div className={`flex flex-col gap-2 min-w-0 flex-1 ${isUser ? 'items-end text-right' : 'items-start text-left'}`}>
